...
export class CadastroTurmaComponent implements OnInit {
  ...
  cadastroNovaTurma(Id:string){
    let turma = new Turma;
    turma.id = Id;
    this.pessoaService.getPessoa(this.cpfAtual).then(value=>turma.instrutores.push(value)).catch();
    this.pessoaService.getPessoa(this.cpfAtual).then(value=>turma.alunos.push(value)).catch();
