...
defineSupportCode(function ({ Given, When, Then }) {
  ...
  Given(/^A turma "([^\"]*)" teve "([^\"]*)" erros no roteiro de "([^\"]*)" com ID "([^\"]*)"/, (turmaId, noAcertos, nomeRoteiro, roteiroId) => {
  ...
  Then('Eu consigo ver que a turma {string} teve {string} erros no roteiro de {string}com ID {string}', function (string, string2, string3, string4) {
      // Write code here that turns the phrase above into concrete actions
  });
});
