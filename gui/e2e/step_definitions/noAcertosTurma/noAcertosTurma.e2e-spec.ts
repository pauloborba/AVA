...
defineSupportCode(function ({ Given, When, Then }) {
    ...
    Given(/^A turma "([^\"]*)" teve "([^\"]*)" acertos no roteiro de "([^\"]*)" com ID "([^\"]*)"/, (turmaId, noperguntas, nomeRoteiro, roteiroId) => {
    });
    ...
    Then(/^Eu consigo ver que a turma "([^\"]*)" teve "([^\"]*)" acertos no roteiro de "([^\"]*)" com ID "([^\"]*)"/, function (turma, perguntas, roteiro, id) {
        expect(auxpage.acertosById(`${id}`)).to.eventually.equal(`Número de Acertos: ${perguntas}`);
    });
});